...
DEFINE_FLASH_FORWARD_KERNEL(flash_fwd_kernel, bool Is_dropout, bool Is_causal, bool Is_local, bool Has_alibi, bool Is_even_MN, bool Is_even_K, bool Return_softmax, bool Is_seq_len_headwise) {
    #if defined(ARCH_SUPPORTS_FLASH)
        static_assert(!(Is_causal && Is_local)); // Enforce constraints
        flash::compute_attn<Kernel_traits, Is_dropout, Is_causal, Is_local, Has_alibi, Is_even_MN, Is_even_K, Return_softmax, Is_seq_len_headwise>(params);
    #else
...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    const bool is_seq_len_headwise = params.is_seq_len_headwise;
    BOOL_SWITCH(is_even_MN, IsEvenMNConst, [&] {
        EVENK_SWITCH(is_even_K, IsEvenKConst, [&] {
            LOCAL_SWITCH((params.window_size_left >= 0 || params.window_size_right >= 0) && !Is_causal, Is_local, [&] {
                BOOL_SWITCH(return_softmax, ReturnSoftmaxConst, [&] {
                    BOOL_SWITCH(is_seq_len_headwise, IsSeqLenHeadwise, [&]{
                        ALIBI_SWITCH(params.alibi_slopes_ptr != nullptr, Has_alibi, [&] {
                                                ...
                                                auto kernel = &flash_fwd_kernel<Kernel_traits, 
                                                    ...
                                                    IsEvenKConst, 
                                                    ReturnSoftmaxConst && Is_dropout, IsSeqLenHeadwise>;
